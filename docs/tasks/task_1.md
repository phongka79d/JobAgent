...
- README status: root `README.md` was not found when this task file was created.
- Project purpose and current run/build/test commands could not be imported from a root README.
- The current workspace contains planning documents but no documented runnable implementation baseline.
- Git metadata was not usable when this task file was created, so current tracked-file and ignore-state claims could not be verified.
- No README conflict was found because no root README exists. `docs/plans/Plan_1.md` and its controlling references in `docs/plans/Master_plan.md` therefore supply all task requirements.
...
- Root `README.md` is not an authoritative source for this task because it is missing.
...
| Batch | Outcome | Exit evidence |
|---|---|---|
| Mandatory Batch01 | Safe Phase 0 scaffold and prerequisite inventory | Exact three-folder scaffold, root configuration boundary, ignored private inputs, and readiness record |
| Mandatory Batch02 | Astryx compatibility locked | Pinned version plus public import/composition matrix for every required component |
| Mandatory Batch03 | ShopAIKey compatibility locked | Safe diagnostic evidence for all six provider capabilities |
| Mandatory Batch04 | pypdf extraction behavior locked | Comparable normal/layout metrics and verified image-only classification |
| Mandatory Batch05 | Embedding model locked | Comparable seeded quality/CPU measurements and selected model/dimension |
| Mandatory Batch06 | Phase 0 decisions consolidated | Complete all-pass report, pinned decisions, clean scaffold, and Plan 2 handoff |
...
- [ ] (01A): Verify local tools and user-owned gate inputs
  ...
  - Details: Establish a safe prerequisite inventory without fabricating provider access, private fixtures, or labels.
  - Dependencies: None
  ...
  - Agent Work: Check tool versions and input presence, record only safe status metadata, and map missing inputs to the affected later tasks.
  - Specific Steps:
  ...
  - Acceptance: Every prerequisite has evidence-backed ready, missing, or blocked status; no secret or private content appears in output.
  - Validation: Re-run the version and presence checks; inspect captured output for configured secret values, Authorization text, provider headers, filenames containing PII, and raw document content.
...
- [ ] (01B): Create the exact three-folder Phase 0 scaffold
  ...
  - Details: Establish the minimum directory and placeholder structure required by the compatibility work.
  - Dependencies: (01A)
  - User Action: None
  - Agent Work: Search current paths first, preserve aligned files, and add only missing Phase 0 directories and minimal placeholders.
  - Specific Steps:
  ...
  - Acceptance: All required Phase 0 paths exist; no fourth product working folder or production behavior was added.
  - Validation: Compare the filesystem against Plan 1 section 6 and master-plan section 5; enumerate non-hidden root directories and classify documentation/metadata directories separately from product working folders.
...
- [ ] (01C): Establish single-root configuration and ignore boundaries
  ...
  - Details: Create the safe configuration contract required by diagnostics without writing real values.
  - Dependencies: (01B)
  ...
  - Agent Work: Add placeholder variable names, enforce ignore rules, and reject separate environment-file drift.
  - Specific Steps:
  ...
  - Acceptance: Required variable names are documented once; real secrets/private data are ignored; no backend secret is exposed to frontend configuration.
  - Validation: Run ignore checks when Git metadata is usable, plus direct filesystem and secret-pattern searches; record the Git-metadata limitation when tracked-file proof is unavailable.
...
- [ ] (01D): Create evaluation manifests and an evidence-first report skeleton
  ...
  - Details: Provide a single evidence destination and safe metadata contracts for later benchmark tasks.
  - Dependencies: (01C)
  ...
  - Agent Work: Create focused manifest templates and report headings without inserting fabricated results.
  - Specific Steps:
  ...
  - Acceptance: Every later gate has a structured evidence location; no pending gate is pre-marked `PASS`; committed metadata contains no private content.
  - Validation: Inspect manifests and report for required fields, safe paths, pending results, and absence of real document text or PII.
...
- [ ] (02A): Select, pin, and initialize a stable Astryx release
  ...
  - Details: Establish a reproducible design-system baseline and preserve only initialization artifacts required by the gate.
  ...
  - Agent Work: Inspect existing frontend package state, select a non-prerelease release using package evidence, pin it exactly, run the required initializer, and record the result.
  - Specific Steps:
  ...
  - Acceptance: npm resolves the exact pin, the required initializer completes, the report records the package/CLI version, and no product flow or floating version was added.
  - Validation: Run the package-manager resolution command, inspect the lockfile for a single exact Astryx version, and compare generated paths against the Phase 0 scope.
...
- [ ] (02B): Build the complete Astryx public component matrix
  ...
  - Details: Resolve the narrower Plan 1 list against the controlling master-plan UI matrix by validating the complete union of sixteen required components.
  - Dependencies: (02A)
  - User Action: None
  - Agent Work: Run the pinned CLI documentation command for each component and capture only public, version-specific evidence.
  - Specific Steps:
  ...
  - Acceptance: All sixteen required component needs have either a verified public import or a documented public composition path; no undocumented internal is referenced.
  - Validation: Re-run each single-component documentation command and cross-check every matrix row against its captured output.
...
- [ ] (02C): Validate and lock the Astryx compatibility decision
  ...
  - Details: Turn the documentation matrix into an evidence-backed `PASS` or `FAIL` gate without building product screens.
  - Dependencies: (02B)
  ...
  - Agent Work: Add the smallest import/composition resolution check, execute it against the exact pin, record the decision, and remove unnecessary initializer artifacts.
  - Specific Steps:
  ...
  - Acceptance: Gate is `PASS` with complete evidence, or honestly `FAIL` with Plan 2 blocked and no undocumented workaround.
  - Validation: Run the focused public-export resolution check and package-manager resolution command; search for internal package paths or invented component APIs.
...
- [ ] (03A): Build the secure isolated diagnostic foundation
  ...
  - Details: Establish a typed, deterministic diagnostic harness whose results can be reported safely and whose provider calls can be faked in normal tests.
  ...
  - Agent Work: Search for existing configuration/redaction/result-schema logic, reuse it where aligned, and implement only the missing diagnostic shell.
  - Specific Steps:
  ...
  - Acceptance: The diagnostic starts only with valid required configuration, produces deterministic sanitized result records, and can be tested without a real provider call.
  - Validation: Run configuration and redaction tests with sentinel secrets; confirm sentinel values and sensitive header names never appear in stdout, stderr, logs, exceptions, or report output.
...
- [ ] (03B): Add model-discovery and basic-completion checks
  ...
  - Details: Implement the checks that (03F) will use to prove that the configured endpoint exposes the master-locked model and accepts a minimal completion request.
  - Dependencies: (03A)
  ...
  - Agent Work: Add focused model-listing and minimal completion capability functions with safe, bounded evidence.
  - Specific Steps:
  ...
  - Acceptance: Fake-provider tests prove correct classification of `gpt-4o-mini`, equivalent-only, missing-model, non-empty-response, and empty-response cases without claiming live compatibility.
  - Validation: Fake-provider tests cover `gpt-4o-mini` present, model absent, equivalent requiring source revision, empty response, and safe error output.
...
- [ ] (03C): Add function-call and tool-result round-trip checks
  ...
  - Details: Implement the complete tool-call-loop check that (03F) will execute against the live provider.
  - Dependencies: (03A)
  - User Action: None
  - Agent Work: Add one harmless synthetic tool contract and validate both the initial call and the follow-up response.
  - Specific Steps:
  ...
  - Acceptance: Fake-provider tests prove that the check accepts the expected tool name, valid typed arguments, and final response, and rejects every named failure branch without claiming a live result.
  - Validation: Fake-provider tests cover malformed JSON, wrong tool, missing tool call, multiple unexpected calls, missing final response, and safe failures.
...
- [ ] (03D): Determine one reliable structured-schema mode
  ...
  - Details: Implement bounded structured-output strategies and validation so (03F) can characterize live behavior and lock one reproducible mode without adding a fallback provider.
  - Dependencies: (03A)
  ...
  - Agent Work: Exercise permitted schema strategies in a bounded order, validate with Pydantic v2, and count repair requests explicitly.
  - Specific Steps:
  ...
  - Acceptance: Fake-provider tests prove that permitted modes, Pydantic validation, the approved reliability rule, and the one-repair limit are enforced without claiming a live selected mode.
  - Validation: Fake-provider tests cover valid output, first-response repair success, second failure, strict incompatibility, invalid types, and repair-limit enforcement.
...
- [ ] (03E): Characterize streaming and enforce diagnostic exit behavior
  ...
  - Details: Implement streaming classification and exit aggregation so (03F) can complete the live compatibility matrix unambiguously.
  - Dependencies: (03A), (03C), (03D)
  - User Action: None
  - Agent Work: Add bounded streaming observation, classify its outcome, and centralize process exit semantics.
  - Specific Steps:
  ...
  - Acceptance: Fake-provider tests prove correct supported/unsupported/unknown classification, non-zero required-failure exits, and sanitized output without claiming live streaming behavior.
  - Validation: Fake-provider tests cover ordered chunks, out-of-order/empty chunks, explicit unsupported response, unknown failure, and exit-code aggregation.
...
- [ ] (03F): Execute the live provider smoke test and lock ShopAIKey decisions
  ...
  - Details: Run the tested diagnostic once as the authorized live compatibility operation and preserve only sanitized aggregate evidence.
  ...
  - Agent Work: Run fake-based tests first, execute the live diagnostic, scan all output for leakage, and write the locked provider decisions to the report.
  - Specific Steps:
  ...
  - Acceptance: `gpt-4o-mini` completes the required completion, function-call, tool-result, and structured-schema checks under the current source; streaming is known; diagnostic exits zero; leakage scan passes. An equivalent can pass only after the controlling adapter decision is revised.
  - Validation: Focused fake-provider tests, live single-purpose diagnostic, exit-code check, and exact secret/prohibited-output scan.
...
- [ ] (04A): Freeze the PDF fixture manifest and pass criterion
  ...
  - Details: Establish immutable fixture IDs, privacy classification, and the pass rule before observing benchmark results.
  - Dependencies: (01C), (01D)
  ...
  - Agent Work: Validate fixture count/types and record safe metadata without opening private content for reporting or changing the corpus after results are known.
  - Specific Steps:
  ...
  - Acceptance: Corpus size/type requirements and the one-redacted-real-CV requirement are met, private paths are ignored, and the pass rule predates benchmark results.
  - Validation: Manifest schema/count check, file-type/page-read smoke check, ignore-state check, and timestamp/order review for the pre-recorded criterion.
...
- [ ] (04B): Implement the focused pypdf benchmark recorder
  ...
  - Details: Create one focused, repeatable local benchmark path with structured aggregate output.
  - Dependencies: (04A)
  - User Action: None
  - Agent Work: Search for an existing evaluation runner, reuse its safe patterns, and add only the pypdf modes and metrics that are missing.
  - Specific Steps:
  ...
  - Acceptance: Both modes run through one consistent path, every record has all required fields, and no raw text or OCR path is emitted.
  - Validation: Tests with safe synthetic text PDFs, malformed inputs, and an image-only fixture; schema validation for required metrics; search for OCR/fallback dependencies.
...
- [ ] (04C): Run the digital-PDF comparison and select a parser mode
  ...
  - Details: Produce comparable measurements and choose one evidence-backed digital-PDF extraction mode.
  - Dependencies: (04B)
  ...
  - Agent Work: Execute both modes, validate result completeness, compare outcomes, and record a single selected mode or an honest failure.
  - Specific Steps:
  ...
  - Acceptance: Comparable results exist for every digital fixture in both modes and one mode meets the approved pass criterion.
  - Validation: Result-set equality check, required-field/schema check, threshold calculation, repeated spot run for deterministic classification, and raw-text leakage scan.
...
- [ ] (04D): Verify exact image-only failure behavior and close the PDF gate
  ...
  - Details: Prove the downstream-visible no-text rule in both benchmark modes and finalize the pypdf compatibility decision.
  ...
  - Agent Work: Execute both normal and layout modes on the image-only fixture, verify exact classification in each, scan dependencies for OCR, and record the selected-mode Phase impact.
  - Specific Steps:
  ...
  - Acceptance: Both modes reproducibly return the exact outcome, no OCR path exists, and the selected digital mode also satisfies (04C).
  - Validation: Focused exact-code assertions for normal and layout modes, dependency/source search for OCR or alternate parsers, and repeated image-only runs.
...
- [ ] (05A): Freeze the seeded retrieval subset and decision protocol
  ...
  - Details: Freeze the Phase 0 subset, seed, metrics, hardware record, pass baselines, and any allowed MiniLM materiality rule before model results are observed.
  - Dependencies: (01D)
  ...
  - Agent Work: Validate safe aggregate metadata, freeze the validation subset, and document the comparison protocol without accessing held-out outcomes.
  - Specific Steps:
  ...
  - Acceptance: Both models will receive the identical validation slice from the locked seeded 60/20/20 split, use `0`-`3` labels and identical metrics, and apply pre-recorded quality/latency baselines while held-out data remains uninspected.
  - Validation: Manifest/schema check, seed/split integrity check, timestamp/order review for decision criteria, and confirmation that no held-out metrics were loaded.
...
- [ ] (05B): Implement the focused two-model CPU benchmark
  ...
  - Details: Create one repeatable runner that normalizes inputs and measurement boundaries while preserving model-specific required prefixes.
  - Dependencies: (05A)
  - User Action: Provide local model-download/cache access when the locked candidates are not already available.
  - Agent Work: Search for existing embedding/evaluation utilities, reuse aligned metrics, and implement only the missing two-model comparison logic.
  - Specific Steps:
  ...
  - Acceptance: Runner accepts only the two locked candidates, produces every required metric, uses CPU, and treats the seeded subset identically apart from required model preprocessing.
  - Validation: Safe synthetic tests for metric calculation, percentile calculation, dimension capture, candidate allowlist, E5 prefixes, deterministic seed handling, and private-text suppression.
...
- [ ] (05C): Execute comparable CPU benchmarks for both candidates
  ...
  - Details: Produce a complete, reproducible comparison table without changing inputs or criteria between runs.
  - Dependencies: (05B)
  - User Action: None after required local inputs/model artifacts are available.
  - Agent Work: Run both candidates under the same recorded conditions, validate coverage, and preserve sanitized aggregate results.
  - Specific Steps:
  ...
  - Acceptance: Both rows are complete and comparable, and no held-out metric or raw private text appears in evidence.
  - Validation: Input-fingerprint equality, required-field/schema check, metric recomputation spot check, candidate allowlist check, and held-out/private-data access scan.
...
- [ ] (05D): Select and lock the embedding model and dimension
  ...
  - Details: Apply the pre-registered rule mechanically and produce one downstream model/dimension contract.
  - Dependencies: (05C)
  ...
  - Agent Work: Evaluate both rows against the pre-recorded quality/latency baselines, consider the required peak-memory measurement in the approved comparison rule, apply E5 preference/materiality logic, and record `PASS` or `FAIL` with Phase impact.
  - Specific Steps:
  ...
  - Acceptance: Exactly one quality/latency-passing model/dimension is selected through the pre-registered rule with peak memory recorded, or the gate honestly fails and blocks Plan 2.
  - Validation: Independent recalculation of quality/latency baselines and the materiality rule, peak-memory record check, selected-model allowlist check, dimension consistency check, and source/report cross-check.
...
- [ ] (06A): Assemble feasibility evidence and a provisional decision table
  ...
  - Details: Convert gate-level aggregate outputs into one auditable draft without copying raw provider payloads, private texts, or unverified claims.
  ...
  - Agent Work: Cross-check source requirements against gate artifacts, reconcile inconsistent gate-level status, and draft a clearly provisional decision table at the physical end of the report.
  - Specific Steps:
  ...
  - Acceptance: Every draft claim traces to a sanitized artifact/command, all required columns are populated, no incomplete gate is marked `PASS`, and no final Phase 0 completion claim is made.
  - Validation: Source-to-report requirement checklist, artifact existence check, heading/order check, provisional-table EOF check, gate-status consistency check, and secret/private-data scan.
...
- [ ] (06B): Lock versions, remove temporary artifacts, and run global safety checks
  ...
  - Details: Leave a minimal reproducible handoff surface rather than an accumulation of Phase 0 experiments.
  - Dependencies: (06A)
  ...
  - Agent Work: Pin exact compatible decisions, remove unused artifacts/dependencies, document only real commands, and execute the global validation matrix.
  - Specific Steps:
  ...
  - Acceptance: Only gate-required artifacts/dependencies remain, all reproducible decisions are pinned, validation evidence is complete, and no secret/private data is exposed.
  - Validation: Backend/frontend resolution checks, focused tests, all single-purpose gate commands, filesystem structure audit, dependency/source searches, secret scan, and Git tracked/ignore audit when available.
...
- [ ] (06C): Enforce the Phase 0 exit gate and produce the Plan 2 handoff
  ...
  - Details: Make phase progression a mechanical consequence of the evidence rather than a narrative assertion.
  - Dependencies: (06B)
  ...
  - Agent Work: Finalize the table from post-cleanup evidence, set Phase 0 status, prepare the exact handoff, and preserve the final decision table at report EOF.
  - Specific Steps:
    ...
    5. Update README Phase 0 status and the task progress tracker from evidence, remove the provisional marker, then re-check final report-table EOF and source consistency.
  ...
  - Acceptance: Plan 2 is authorized only when every required gate is `PASS`; otherwise the blocking reason and adapter-only repair scope are explicit.
  - Validation: Final decision-table all-pass assertion, handoff completeness checklist, progress-tracker synchronization, README/report status comparison, and out-of-scope/fallback search.
  ...
  - Files: `backend/evaluation/reports/phase_0_feasibility.md`, `README.md`, dependency/configuration locks, this progress tracker when execution status is updated.
...
- [ ] The progress tracker exactly matches all detailed task IDs and status changes.
