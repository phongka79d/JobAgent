...
def test_public_app_endpoints_match_current_phase(healthy_client: TestClient) -> None:
    ...
    # Plan 4 Phase 3 application surface is exactly seven authorized routes.
    ...
    assert not any(any(marker in path for marker in ("jobs", "match", "synthetic")) for path in paths)
